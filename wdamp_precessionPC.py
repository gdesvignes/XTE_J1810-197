...
def get_L(cube, nQ, nU, xm, Qm, Um, mjds, have_EFAC, nEFAC, rcvr_lut):
        ...
        for ii in range(nfiles):
            ...
            Phi=np.pi/2-omegap*(mjds[ii]-t0)-Phi0
            # the evolution of magnetic inclination angle
            alpha=np.arccos(np.sin(theta)*np.sin(Phi)*np.sin(chi)+np.cos(theta)*np.cos(chi))
        ...
        return -0.5 * chi2 -0.5*logdet, betas
...
class Precessnest():
    ...
    def set_labels(self):
        ...
        self.labels.extend(["theta0"])
        self.labels.extend(["chi"])
        self.labels.extend(["Phi0"])
        self.labels.extend(["epsilon"])
    ...
    def Prior(self, cube):
        ...
        pcube[ipar] = cube[ipar]*(180); ipar += 1
        pcube[ipar] = cube[ipar]*(180); ipar += 1
        pcube[ipar] = cube[ipar]*(360); ipar += 1
        pcube[ipar] = cube[ipar]*(5e-6) + 1e-7; ipar += 1
        pcube[ipar] = cube[ipar]*(200) + 5; ipar += 1
        ...
        # EFAC
        if self.have_EFAC:
            pcube[ipar:ipar+self.nEFAC] = cube[ipar:ipar+self.nEFAC]*4+.3
        return pcube
...
settings.file_root = 'wdamp_precess'
