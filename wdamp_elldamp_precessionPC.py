...
def get_L(cube, nQ, nU, xm, Qm, Um, mjds, have_EFAC, nEFAC, rcvr_lut):
        ...
        for ii in range(nfiles):
            ...
            if have_EFAC:
                EFAC = cube[nmpar+2*nfiles+rcvr_lut[ii]]
            else:
                EFAC = 1.
...
class Precessnest():
    ...
    def Prior(self, cube):
        ...
        # EFAC
        if self.have_EFAC:
            pcube[ipar:ipar+self.nEFAC] = UniformPrior(1.3, 3)  (cube[ipar:ipar+self.nEFAC])
        return pcube
...
settings.file_root = 'wdamp_elldamp4'
