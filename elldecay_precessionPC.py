...
def get_L(cube, nQ, nU, xm, Qm, Um, mjds, have_EFAC, nEFAC, rcvr_lut):
        ...
        #t0 = cube[6]
        t0 = 58445
        phi0s = np.deg2rad(cube[6:6+nfiles])
        psi0s = np.deg2rad(cube[6+nfiles:6+2*nfiles])
...
class Precessnest():
    ...
    def Prior(self, cube):
        ...
        # Zeta
        #
        pcube[ipar] = GaussianPrior(160,2) (cube[ipar]); ipar +=1 # GD
        pcube[ipar] = UniformPrior(1,42) (cube[ipar]); ipar += 1
        pcube[ipar] = UniformPrior(140,180) (cube[ipar]); ipar += 1
        pcube[ipar] = UniformPrior(30,360) (cube[ipar]); ipar += 1

        pcube[ipar] = LogUniformPrior(5, 180) (cube[ipar]); ipar += 1
        pcube[ipar] = LogUniformPrior(5, 180) (cube[ipar]); ipar += 1
        #pcube[ipar] = cube[ipar]*(55) + 58430; ipar += 1

        for ii in range(self.nfiles):
            pcube[ipar+ii] = GaussianPrior((self.pPh[1][ii]+self.pPh[0][ii])/2., (self.pPh[1][ii]-self.pPh[0][ii])/4.) (cube[ipar+ii]);
        ipar += self.nfiles
        for ii in range(self.nfiles):
            pcube[ipar+ii] = GaussianPrior((self.pPs[1][ii]+self.pPs[0][ii])/2., (self.pPs[1][ii]-self.pPs[0][ii])/4.) (cube[ipar+ii])
        ipar += self.nfiles
